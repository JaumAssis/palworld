...
function MyCollection() {
  ...
  return (
    ...
    <div style={{ padding: 'var(--sp-lg)', maxWidth: 'min(90vw, 760px)', margin: '0 auto' }}>
      <Link to="/"><button className="sign-button sign-button-fluid" style={{ marginBottom: '12px' }}>{t('backToMenu')}</button></Link>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginBottom: '14px' }}>
        <h1 style={{
          margin: 0, lineHeight: 1.2, fontFamily: "'Rye', Georgia, serif", fontSize: 'var(--fs-xl)',
          color: '#f3e2b3', WebkitTextStroke: '1px #2b160a',
          textShadow: '2px 2px 0 #000, 0 0 14px rgba(0,0,0,0.6)'
        }}>{t('collectionTitle')}</h1>
        <p style={{ color: '#d9c4a3', margin: 0, fontSize: 'var(--fs-sm)', textShadow: '1px 1px 2px rgba(0,0,0,0.7)' }}>{t('uniqueCardsCollected', { owned: ownedUnique, total: totalUnique })}</p>
      </div>

      {/* ---------- BUSCA + "SÓ AS QUE EU TENHO" ---------- */}
      <div style={{ display: 'flex', gap: 'var(--sp-sm)', marginBottom: '14px', alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{
          flex: '1 1 200px', display: 'flex', alignItems: 'center', gap: '6px',
          background: '#fdf6e3', border: '2px solid #d4a76a', borderRadius: '8px',
          padding: 'var(--sp-xs) var(--sp-sm)', boxShadow: 'inset 0 1px 3px rgba(0,0,0,0.15)'
        }}>
          ...
          <input
            ...
            style={{
              border: 'none', background: 'transparent', outline: 'none', fontSize: 'var(--fs-sm)',
              width: '100%', color: '#5c3418'
            }}
          />
        </div>
        <label style={{ fontSize: 'var(--fs-2xs)', display: 'flex', alignItems: 'center', gap: '4px', color: '#f3e2b3', textShadow: '1px 1px 2px rgba(0,0,0,0.7)' }}>
      ...
      <div style={{
            ...
            return (
              <div key={type} onClick={() => changeFilterType(type)}
                   style={{
                     ...
                     padding: 'var(--sp-2xs) var(--sp-2xs)', textAlign: 'center', borderRadius: '0 8px 8px 0',
                     background: TAB_COLORS[type], color: '#fff3d6',
                     fontSize: 'var(--fs-2xs)', fontWeight: 700, letterSpacing: '0.3px',
        ...
        <div style={{
          background: '#fdf6e3', borderRadius: '10px', padding: 'var(--sp-md) var(--sp-md)', marginLeft: '14px',
            ...
            {pageCards.map(card => {
              ...
              return (
                <div key={card.card_number}
                     ...
                     style={{
                  textAlign: 'center', border: '1px solid #ddd', borderRadius: '8px', padding: 'var(--sp-2xs)',
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    {owned && (
                      <span style={{
                        ...
                        fontSize: 'var(--fs-2xs)', fontWeight: 700, padding: '1px 5px', borderRadius: '5px'
                  ...
                  <p style={{ fontSize: 'var(--fs-2xs)', margin: '3px 0 0', color: owned ? '#000' : '#999' }}>{card.name}</p>
                </div>
              )
            })}
            {/* preenche slots vazios na última página, mantendo o layout 3x3 */}
            {Array.from({ length: PAGE_SIZE - pageCards.length }).map((_, i) => (
              <div key={'empty' + i} style={{ border: '1px dashed #e0d0b0', borderRadius: '8px', minHeight: 'clamp(80px, 9vw, 130px)' }} />
            ))}
          </div>

          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '16px', marginTop: '12px' }}>
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} style={{ fontSize: 'var(--fs-sm)' }}>{t('prevPage')}</button>
            <span style={{ fontSize: 'var(--fs-2xs)', color: '#777' }}>{t('pageOf', { current: currentPage, total: totalPages })}</span>
            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} style={{ fontSize: 'var(--fs-sm)' }}>{t('nextPage')}</button>
          </div>
        </div>
      </div>
      ...
      {selectedCard && (
        <div onClick={() => setSelectedCard(null)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            background: '#fff', borderRadius: '16px', padding: 'var(--sp-lg)', textAlign: 'center', maxWidth: 'var(--panel-w-xs)'
          }}>
            <img src={selectedCard.card.image_url} alt={selectedCard.card.name}
                 style={{
                   width: 'clamp(200px, 20vw, 300px)', borderRadius: '10px', boxShadow: '0 8px 24px rgba(0,0,0,0.4)',
                   filter: selectedCard.owned ? 'none' : 'grayscale(100%)',
                   opacity: selectedCard.owned ? 1 : 0.5
                 }} />
            <h3 style={{ margin: '12px 0 4px', fontSize: 'var(--fs-lg)' }}>{selectedCard.card.name}</h3>
            <p style={{ color: '#777', fontSize: 'var(--fs-sm)', margin: '0 0 8px' }}>
              {selectedCard.owned ? t('ownedCopies', { qty: Math.min(selectedCard.qty, 4) }) : t('notOwnedYet')}
            </p>
            {selectedCard.owned && selectedCard.reserved > 0 && (
              <p style={{ color: '#8a5a2b', fontSize: 'var(--fs-2xs)', margin: '0 0 8px' }}>
            ...
            {(() => {
              ...
              return (
                <>
                  <p style={{ fontSize: 'var(--fs-2xs)', color: '#555', margin: '0 0 12px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
                    <button
                      ...
                      disabled={disabled}
                      style={{ padding: 'var(--sp-sm) var(--sp-lg)', fontSize: 'var(--fs-sm)', fontWeight: 600, opacity: disabled ? 0.5 : 1 }}>
                      {t('craftBtn', { cost })}
                    </button>
                    <button onClick={() => { setSelectedCard(null); setCraftMsg('') }} style={{ padding: 'var(--sp-sm) var(--sp-lg)', fontSize: 'var(--fs-sm)' }}>{t('close')}</button>
                  </div>
                  {!cost && <p style={{ fontSize: 'var(--fs-2xs)', color: '#999', marginTop: '8px' }}>{t('cannotCraftRarity')}</p>}
                  {atMax && cost && <p style={{ fontSize: 'var(--fs-2xs)', color: '#999', marginTop: '8px' }}>{t('alreadyMaxCopies')}</p>}
                  {craftMsg && <p style={{ fontSize: 'var(--fs-2xs)', color: 'red', marginTop: '8px' }}>{craftMsg}</p>}
                </>
              )
            })()}
          </div>
